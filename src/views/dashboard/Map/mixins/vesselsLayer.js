...
import { getVesselIcon, prettyDateForm } from '../helpers'
    ...
    vesselMarkerMouseover (marker, vessel) {
      ...
      axios.get(`map/vessel/${vessel[0]}`)
        .then(res => {
          ...
          const content = `${data.name} [${data.imo || '--'}] / ${data.speed || '--'} knots / ${data.course || '--'}&deg;<br>AIS Status: ${data.nav_status}<br>Position received: ${prettyDateForm(new Date(data.timestamp))}<br>Destination: ${data.destination || 'Unknown'}<br>ETA: ${data.eta || 'Unknown'}`
